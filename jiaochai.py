...
def OpenFile(system):
    """Opens the BPA file."""
    # path_win = 'C:\\Users\\Outrace\\Desktop\\bpa4.1_crack\\SAMPLE\\IEEE9\\IEEE90.DAT' 
    path_win = 'C:\\Users\\Outrace\\Desktop\\bpa4.1_crack\\SAMPLE\\039bpa.DAT' 
    ...
    for i in range(0, len(bpa_str_ar)):
        ...
        if line[0] == 'B':
            ...
            if line[1] == ' ' or line[1] == 'T' or line[1] == 'C' or line[1] == 'V' or line[1] == 'F' or line[1] == 'J' or line [1] == 'X':
                ...
                generator.ip_ctrl = 0;  #PV
                generator.iv_mode = 1;
                generator.Pmax_uc = myFloat(line[38-chinese_count:42-chinese_count], 99999, 0)
                generator.pgini = float(line[42-chinese_count:47-chinese_count]) * P_generator_m
                ...
                generator.usetp = myFloat(line[57-chinese_count:61-chinese_count], 1, 3)
                ...
                generator.iv_mode = 1;
                generator.pgini = myFloat(line[42-chinese_count:47-chinese_count], 0, 0)
                generator.cQ_max = myFloat(line[47-chinese_count:52-chinese_count], 9999, 0)
                generator.cQ_min = myFloat(line[52-chinese_count:57-chinese_count], -9999, 0)
                generator.Pmax_uc = myFloat(line[38-chinese_count:42-chinese_count], 99999, 0)
                generator.usetp = myFloat(line[57-chinese_count:61-chinese_count], 0, 3)
                generator.pmaxratf = generator.Pmax_uc / MVABASE
...
def GetTCard(bpa_file, bpa_str_ar):
    ...
    for i in range(0, len(bpa_str_ar)):
        ...
        if line[0] == 'T':
            ...
            hv = line[62-chinese_count_from-chinese_count_to:67-chinese_count_from-chinese_count_to]
            lv = line[67-chinese_count_from-chinese_count_to:72-chinese_count_from-chinese_count_to]
            if isfloat(hv) and isfloat(lv):
                if myFloat(hv, 0, 2) > myFloat(lv, 0, 2):
                    hv = myFloat(hv, 0, 2); lv = myFloat(lv, 0, 2)
                else:
                    temp = lv; lv = myFloat(hv, 0, 2); hv = myFloat(temp, 0, 2)
            TypTr.utrn_h = hv
            TypTr.utrn_l = lv
